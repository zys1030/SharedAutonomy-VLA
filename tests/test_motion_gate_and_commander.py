...
def test_load_motion_enable_config_prefers_local_file(tmp_path: Path) -> None:
    path = tmp_path / "manual_cartesian.local.yaml"
    path.write_text("enable_motion: false\n", encoding="utf-8")

    enabled, source = load_motion_enable_config(
        cli_config_enable_motion=True,
        config_path=path,
    )
...
def test_load_motion_enable_config_falls_back_without_local_file(tmp_path: Path) -> None:
    missing = tmp_path / "missing.yaml"

    assert load_motion_enable_config(
        cli_config_enable_motion=True,
        config_path=missing,
    ) == (True, "--config-enable-motion")
    assert load_motion_enable_config(
        cli_config_enable_motion=False,
        config_path=missing,
    ) == (False, "built-in safe default (motion disabled)")
...
def test_workspace_from_mapping_and_loader(tmp_path: Path) -> None:
    ...
    lowered = workspace_from_mapping({**mapping, "min_flange_z_m": 0.08})
    assert lowered.min_flange_z_m == pytest.approx(0.08)
    assert lowered.table_z_m == pytest.approx(-0.02)


def test_workspace_loader_requires_local_geometry_for_motion_path(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    with pytest.raises(FileNotFoundError, match="Measured Cartesian workspace"):
        load_cartesian_workspace()


def test_workspace_loader_allows_generic_offline_fallback(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    workspace, source = load_cartesian_workspace(allow_example_fallback=True)
    assert source == "example_cartesian_workspace() [offline only]"
    assert workspace.tool_tip_offset_base_m == (0.0, 0.0, 0.0)
