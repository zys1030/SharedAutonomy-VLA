...
WINDOW_KIND_GRASP = "grasp"
WINDOW_KIND_EPISODE_START = "episode_start"


@dataclass(frozen=True, slots=True)
class CriticalFrameConfig:
    """Parameters used to detect and weight chunk-start windows.

    ``window_kind="grasp"`` (default) weights frames around the first
    open-to-close gripper transition.  ``window_kind="episode_start"``
    weights the first ``start_frames`` of every episode instead, so the
    sampler oversamples reach/color-fork chunk starts rather than close/lift.
    """
    ...
    weight: float = 5.0
    window_kind: str = WINDOW_KIND_GRASP
    start_frames: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        kind = str(self.window_kind)
        if kind not in {WINDOW_KIND_GRASP, WINDOW_KIND_EPISODE_START}:
            raise ValueError(
                "window_kind must be 'grasp' or 'episode_start', "
                f"got {self.window_kind!r}"
            )
        if kind == WINDOW_KIND_EPISODE_START and int(self.start_frames) < 1:
            raise ValueError("start_frames must be >= 1 when window_kind is episode_start")
        if int(self.start_frames) < 0:
            raise ValueError("start_frames must be >= 0")
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> CriticalFrameIndex:
        ...
        index = cls(
            ...
            config=CriticalFrameConfig(
                ...
                window_kind=str(config_payload.get("window_kind", WINDOW_KIND_GRASP)),
                start_frames=int(config_payload.get("start_frames", 0)),
            ),
...
def build_critical_frame_index(
    *,
    ...
    if config.window_kind != WINDOW_KIND_GRASP:
        raise ValueError("build_critical_frame_index requires window_kind='grasp'")
...
def build_episode_start_frame_index(
    *,
    dataset_repo_id: str,
    dataset_root: str | Path,
    num_frames: int,
    ...
    """Weight the first ``start_frames`` of each episode as chunk-start windows.

    Unlike :func:`build_critical_frame_index`, this does not read gripper
    actions.  ``close_frame_index`` is set to the episode start so the existing
    sampler JSON schema still has a complete close/window record; the sampler
    only uses ``window_start_index`` / ``window_end_index``.
    """
    ...
    if config.window_kind != WINDOW_KIND_EPISODE_START:
        raise ValueError("build_episode_start_frame_index requires window_kind='episode_start'")

    ranges = tuple(episode_ranges)
    _validate_episode_ranges(ranges, num_frames)

    start_frames = int(config.start_frames)
    episodes: list[CriticalFrameEpisode] = []
    for episode in ranges:
        window_end_index = min(episode.to_index - 1, episode.from_index + start_frames - 1)
        episodes.append(
            CriticalFrameEpisode(
                episode_index=episode.episode_index,
                from_index=episode.from_index,
                to_index=episode.to_index,
                close_frame_index=episode.from_index,
                window_start_index=episode.from_index,
                window_end_index=window_end_index,
            )
        )
