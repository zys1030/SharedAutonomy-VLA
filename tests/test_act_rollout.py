...
from sharedautonomy.robot.gripper import logical_gripper_is_open
...
class _FakeGripperTeleop:
    def __init__(self) -> None:
        self.working_open_fraction = 0.65
        self.pulses: list[float] = []

    def open_to_fraction(self, physical_open_fraction: float) -> float:
        self.pulses.append(float(physical_open_fraction))
        return float(physical_open_fraction)


def test_logical_gripper_hysteresis_ignores_chatter_around_half() -> None:
    assert logical_gripper_is_open(0.47, commanded_open=True) is True
    assert logical_gripper_is_open(0.39, commanded_open=True) is False
    assert logical_gripper_is_open(0.53, commanded_open=False) is False
    assert logical_gripper_is_open(0.60, commanded_open=False) is True


def test_logical_gripper_zero_hysteresis_matches_single_cut() -> None:
    assert logical_gripper_is_open(0.50, commanded_open=True, hysteresis=0.0) is True
    assert logical_gripper_is_open(0.49, commanded_open=True, hysteresis=0.0) is False
    assert logical_gripper_is_open(0.50, commanded_open=False, hysteresis=0.0) is True


def test_threshold_gripper_does_not_repulse_in_hysteresis_band() -> None:
    teleop = _FakeGripperTeleop()
    gripper = ThresholdGripperActuator(teleop=teleop)
    assert gripper.command_open_fraction(0.48) is False
    assert gripper.command_open_fraction(0.39) is True
    assert teleop.pulses == [0.0]
    assert gripper.command_open_fraction(0.55) is False
    assert gripper.command_open_fraction(0.60) is True
    assert teleop.pulses == [0.0, 0.65]


def test_act_rollout_config_rejects_hysteresis_that_leaves_the_unit_interval() -> None:
    with pytest.raises(ValueError, match="hysteresis"):
        ActRolloutConfig(gripper_hysteresis=0.5)
