...
def stamp_cartesian_workspace(*, max_flange_z_m: float | None = 0.45) -> CartesianWorkspace:
    """Day-1 stamp workspace fixture for offline tests and example dry-runs.

    Production runners should load machine-local geometry from
    ``configs/local/rm65_safety.local.yaml`` instead of hard-coding lab values.
    """
        ...
        *,
        ...
        if self.limits.enforce_fixed_orientation:
            try:
                ...
                safe_rpy = self.limits.fixed_ee_rpy_rad
