...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=DEFAULT_INFERENCE_NOISE_SEED,
        help=(
            "Base seed for flow-matching noise at each action-chunk replan "
            f"(chunk N uses seed+N). Default: {DEFAULT_INFERENCE_NOISE_SEED}. "
            "Pass -1 for stochastic inference."
        ),
    )
...
def main() -> int:
    ...
    runtime = SmolVLAInferenceRuntime(
        SmolVLARuntimeConfig(
            ...
            inference_noise_seed=(
                None if int(args.inference_noise_seed) < 0 else int(args.inference_noise_seed)
            ),
        )
    )
    runtime.load()
    ...
    logger.info(
        "Endpoints: GET /health  POST /infer  POST /infer_dataset (state_dim=%s)",
        runtime.describe().get("state_dim"),
    )
    try:
