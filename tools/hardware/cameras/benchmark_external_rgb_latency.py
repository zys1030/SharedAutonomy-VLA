...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--friendly-name", type=str, default=None)
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Direct OpenCV index; otherwise provide a friendly name or VID/PID pair",
    )
...
def select_capture(
    *,
    ...
    if opencv_index is not None:
        candidate_indices = [opencv_index]
        selected_meta = next((d for d in devices if d.get("index") == opencv_index), None)
    else:
        matches = [
            device
        ...
        if not matches:
            raise RuntimeError(
                "No matching external camera found: "
        ...
        candidate_indices = [winrt_index, winrt_index - 1, winrt_index + 1, winrt_index + 2]
    seen: set[int] = set()
...
def main() -> None:
    ...
    if (args.vendor_id is None) != (args.product_id is None):
        raise ValueError("--vendor-id and --product-id must be provided together")
    if (
        args.opencv_index is None
        and not args.friendly_name
        and args.vendor_id is None
    ):
        raise ValueError(
            "Provide --opencv-index, --friendly-name, or both --vendor-id and --product-id"
        )
