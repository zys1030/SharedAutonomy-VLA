...
DEFAULT_GRIPPER_CLOSE_THRESHOLD = 0.5
DEFAULT_GRIPPER_HYSTERESIS = 0.1


def validate_gripper_hysteresis_band(
    *,
    close_threshold: float = DEFAULT_GRIPPER_CLOSE_THRESHOLD,
    hysteresis: float = DEFAULT_GRIPPER_HYSTERESIS,
) -> tuple[float, float]:
    """Return ``(close_threshold, hysteresis)`` after checking the Schmitt band."""
    center = float(close_threshold)
    band = float(hysteresis)
    if not 0.0 < center < 1.0:
        raise ValueError("close_threshold must be in (0, 1)")
    if band < 0.0:
        raise ValueError("hysteresis must be >= 0")
    if center - band <= 0.0 or center + band >= 1.0:
        raise ValueError("close_threshold ± hysteresis must stay in (0, 1)")
    return center, band


def logical_gripper_is_open(
    open_fraction: float,
    *,
    commanded_open: bool,
    close_threshold: float = DEFAULT_GRIPPER_CLOSE_THRESHOLD,
    ...
    """Classify an action gripper value as logical open, with Schmitt hysteresis.

    While open, close only when ``open_fraction < close_threshold - hysteresis``.
    While closed, open only when ``open_fraction >= close_threshold + hysteresis``.
    Values inside the band keep the current commanded state so chatter around
    0.5 does not pulse the hardware. ``hysteresis=0`` matches a single 0.5 cut.
    """
    center, band = validate_gripper_hysteresis_band(
        close_threshold=close_threshold,
        hysteresis=hysteresis,
    )
    fraction = float(open_fraction)
    if commanded_open:
        return fraction >= (center - band)
    return fraction >= (center + band)
