...
def test_episode_start_index_weights_prefix_not_grasp() -> None:
    ranges = (
        EpisodeFrameRange(0, 0, 10),
        EpisodeFrameRange(1, 10, 16),
    )
    ...
    assert index.config.window_kind == WINDOW_KIND_EPISODE_START
    assert index.episodes[0].window_start_index == 0
    assert index.episodes[0].window_end_index == 3
    assert index.episodes[1].window_start_index == 10
    assert index.episodes[1].window_end_index == 13
    assert index.num_window_frames == 8


def test_episode_start_sampler_marks_prefix_starts() -> None:
    pytest.importorskip("torch")
    from sharedautonomy.data.critical_sampler import CriticalFrameSampler
    ...
    index = build_episode_start_frame_index(
        ...
        dataset_root="dataset",
        num_frames=12,
        episode_ranges=(EpisodeFrameRange(0, 0, 12),),
        config=CriticalFrameConfig(
            window_kind=WINDOW_KIND_EPISODE_START,
            start_frames=3,
            weight=5.0,
    ...
    assert list(sampler) == list(range(11))
    assert sampler.critical_frame_count == 3


def test_load_legacy_grasp_index_without_window_kind(tmp_path) -> None:
    actions = [_action(value) for value in [0.8, 0.8, 0.4, 0.4]]
    index = build_critical_frame_index(
        dataset_repo_id="local/test",
        dataset_root=tmp_path / "dataset",
        num_frames=4,
    ...
    del payload["config"]["window_kind"]
    del payload["config"]["start_frames"]
    path.write_text(json.dumps(payload), encoding="utf-8")

    loaded = load_critical_frame_index(path)
    assert loaded.config.window_kind == "grasp"
    assert loaded.config.start_frames == 0
    assert loaded.episodes[0].close_frame_index == 2
