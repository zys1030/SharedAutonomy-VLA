...
try:
    from tools.hardware.spacemouse.benchmark_input_rate import enumerate_spacemice
except ModuleNotFoundError:
    from benchmark_input_rate import enumerate_spacemice


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "DANGER: this verification tool moves RM-65B joint 6. Map SpaceMouse yaw "
            "to guarded J6 low-follow CAN-FD commands. Hold the left button to enable; "
            "releasing it ends the test. Keep the operator at the stop control."
        )
    )
