...
Use ``--reset-every N`` to clear that queue every N steps (ACT-style replan).
``--post-close-hold-s`` is off unless set: after an open→close edge it freezes
arm joints for that many seconds; gripper pulses are not held.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Clear the server action queue every N successful infer steps "
            "before requesting a fresh plan. 0 resets only on the first request "
            "and lets the server consume its configured action queue."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Schmitt band around the 0.5 open/close cut so chatter does not "
            "re-pulse the gripper (0 disables; default 0.1: close <0.4, open >=0.6)"
        ),
    )
    parser.add_argument(
        "--post-close-hold-s",
        ...
        default=0.0,
        help=(
            "After an open-to-close gripper edge, freeze arm joint commands for this many "
            "seconds so the jaw can settle. Disabled by default and not used in the reported "
            "cube evaluations; pass a positive value to enable."
        ),
    )
    parser.add_argument(
        "--go-to-ready",
...
class _ThresholdGripper:
    """Edge-trigger open/close; physical open uses teleop working_open_fraction.

    Matches SpaceMouse teleop: logical open=1.0 / close=0.0 in state, but the
    open pulse is only ``working_open_fraction`` of full travel (not full open).
    Open/close uses Schmitt hysteresis so action chatter around 0.5 does not
    re-pulse the hardware.
    """
    ...
    @property
    def is_open(self) -> bool:
        return bool(self._commanded_open)
...
def main() -> int:
    ...
    validate_gripper_hysteresis_band(
        close_threshold=DEFAULT_GRIPPER_CLOSE_THRESHOLD,
        hysteresis=float(args.gripper_hysteresis),
    )
    post_close_hold_s = validate_post_close_hold_s(float(args.post_close_hold_s))
    ...
    if args.enable_gripper:
        ...
        gripper_handler = _ThresholdGripper(
    ...
    post_close_hold = PostCloseMotionHold(hold_s=post_close_hold_s)
    ...
    try:
            ...
            json.dumps(
                {
                    ...
                    "gripper_hysteresis": float(args.gripper_hysteresis),
                    "post_close_hold_s": post_close_hold_s,
                    ...
                    "server_n_action_steps": health.get("n_action_steps"),
                    "server_chunk_size": health.get("chunk_size"),
        ...
        for step_index in range(total_steps):
            ...
            consecutive_misses = 0
            do_reset = should_reset_action_queue(
            ...
            was_open = bool(gripper_handler.is_open)
            gripper_commanded = gripper_handler.command_open_fraction(gripper_fraction)
            close_triggered = was_open and not bool(gripper_handler.is_open)
            hold = post_close_hold.apply(
                commanded_joints_deg=safe_joints,
                freeze_joints_deg=present_joints.tolist(),
                close_triggered=close_triggered,
                now_s=time.perf_counter(),
            )
            send_joints = hold.joints_deg
            motion_sent = False
            if motion_enabled:
                if commander is None:
                    raise RuntimeError("joint commander is required when motion is enabled")
                commander.send_joint_target(send_joints)
                motion_sent = True
            if gripper_commanded:
                # Logical 0/1 for the next observation.state (matches demo labels);
                # physical reopen already used working_open_fraction above.
                if gripper_actuator is not None:
                    commanded_gripper_fraction = float(gripper_actuator.commanded_open_fraction)
                else:
                    commanded_gripper_fraction = 1.0 if gripper_fraction >= 0.5 else 0.0
                    ...
                    json.dumps(
                        {
                            ...
                            "sent_joints_deg": [round(float(value), 4) for value in send_joints],
                            "gripper_open_fraction": round(gripper_fraction, 4),
                            "gripper_commanded": gripper_commanded,
                            "close_triggered": close_triggered,
                            "post_close_hold_active": hold.active,
                            "post_close_hold_remaining_s": round(float(hold.remaining_s), 3),
