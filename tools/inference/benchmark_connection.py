...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dataset-root",
        type=Path,
        default=Path("outputs/datasets/shape_pick_place_v1"),
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "stage": "warmup_keep_alive_done",
                "rtt_ms": round((time.perf_counter() - t_w) * 1000.0, 1),
            }
        ),
        flush=True,
    )
