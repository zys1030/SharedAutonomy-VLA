...
class SmolVLAInferenceRuntime:
    ...
    def infer_chunk(self, observation: InferObservation) -> np.ndarray:
        ...
        with torch.inference_mode():
            processed = self._preprocessor(batch)
            action_chunk = self._policy.predict_action_chunk(processed)
    ...
    def infer_dataset_frame(
        ...
        *,
        ...
        obs = InferObservation(
            state=np.asarray(sample["observation.state"], dtype=np.float32).reshape(STATE_DIM),
    ...
    @staticmethod
    def _observation_to_batch(observation: InferObservation, torch: Any) -> dict[str, Any]:
