...
class CartesianWorkspace:
    ...
    a measured, machine-local tool-tip offset.
    """
...
def limit_cartesian_target(
    ...
    *,
    ...
    if previous_velocity_m_s is not None and max_acceleration_m_s2 is not None:
        ...
        if max_speed_m_s_per_axis is not None:
            accel_limits = [max(limit / dt, limit / 0.2) for limit in speed_limits]
...
def validate_fixed_orientation(
    ...
    ``axes`` selects which RPY components to check (0=roll, 1=pitch, 2=yaw).
    Default is all three. Tool-yaw teleop passes ``(0, 1)`` so yaw may change.
    """
    ...
    check_axes = (0, 1, 2) if axes is None else tuple(int(axis) for axis in axes)
    if not check_axes:
        raise ValueError("axes must contain at least one RPY index")
    if any(axis not in (0, 1, 2) for axis in check_axes):
        raise ValueError("axes values must be 0, 1, or 2")
    errors = [
        abs(math.remainder(value - origin, 2 * math.pi))
        for origin, value in zip(reference, target, strict=True)
    ]
    if any(errors[axis] > tolerance for axis in check_axes):
